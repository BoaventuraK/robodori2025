...
void setup()
{
  ...
  qtr.setTypeRC();
  qtr.setSensorPins((const uint8_t[]){23, 18, 17, 4, 22}, SensorCount);
...
void PID_Linefollow(int error){
    ...
    lsp = speedLeft - PIDvalue;
    rsp = speedRight + PIDvalue;
...
void motor_drive(int speedLeft, int speedRight){
  
  if(right>0)
  {
    motor2.drive(speedRight, 1000);
    //motor2.forward();
  }
  else 
  {
